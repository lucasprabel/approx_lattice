...
/*
	Sample x in R^m with parameter zeta such that A_m * x = u mod q (using the trapdoor T and h_inv the inverse of the tag for A_m)
		x is in the CRT domain
		A is in the CRT domain
...
void sample_pre_target(poly_matrix x, poly_matrix A_m, poly_matrix T, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, poly h_inv, poly_matrix u)
	...
	printf("z :  \n");
	print_signed_poly_matrix(z, 2, 1);
	printf("\n\n");
...
/*
	Sample x in R^m with parameter zeta such that A_m * x = u + e mod q (using the trapdoor T and h_inv the inverse of the tag for A_m)
		x is in the CRT domain
		A is in the CRT domain
		u is in the CRT domain
		T is in the CRT domain
		...
		h_inv is in the CRT domain
*/
void approx_sample_pre_target(poly_matrix x, poly_matrix A_m, poly_matrix T, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, poly h_inv, poly_matrix u)
	{
	// Sample a perturbation p in R^m
	signed_scalar p_coeffs[PARAM_N * PARAM_M];
	signed_poly_matrix p = p_coeffs;
	...
	// Sample z from the G-lattice with target v
	signed_poly_matrix z = (signed_poly_matrix) poly_matrix_element(x, 1, PARAM_M_BARRE, 0); // store z at the end of x to get TI * z for cheaper
	
	module_sample_G_cut(z, v);
	
	// Make sure z has positive coefficients and put it in the CRT domain
	for(int i = 0 ; i < PARAM_N * PARAM_W ; ++i)
		{
		z[i] += PARAM_Q;
		}
	
	matrix_crt_representation((poly_matrix) z, PARAM_W, 1, LOG_R);
