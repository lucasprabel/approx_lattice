...
/*
	Sample t form the ring G-lattice (included in R^k) with parameter alpha, such that <g,t> = u mod q
*/
void ring_sample_G_cut(signed_poly_matrix t, poly u)
	{
	signed_scalar t2[PARAM_N * PARAM_K];

	ring_sample_G(t2, u);
	
	for (int i = 0 ; i < PARAM_N * (PARAM_K - PARAM_L) ; i++){
		t[i] = t2[PARAM_N * PARAM_L + i];
	}
...
void module_sample_G_cut(signed_poly_matrix t, poly_matrix u)
	{

	signed_scalar t2_coeffs[PARAM_N * PARAM_D * PARAM_K];
	signed_poly_matrix t2 = t2_coeffs;

	module_sample_G(t2, u);

	for (int i = 0 ; i < PARAM_N * PARAM_D * (PARAM_K - PARAM_L) ; i++){
		t[i] = t2[PARAM_N * PARAM_D * PARAM_L + i];
	}
...
void approx_sample_perturb(signed_poly_matrix p, cplx_poly_matrix T, cplx_poly_matrix sch_comp)
	{

	cplx c_coeffs[PARAM_N * 2 * PARAM_D], cplx_p_coeffs[PARAM_N * PARAM_D * (PARAM_K - PARAM_L)];
	cplx_poly_matrix c = c_coeffs, cplx_p = cplx_p_coeffs;
	...
	matrix_cplx_crt_representation(cplx_p, PARAM_D * (PARAM_K - PARAM_L), 1);
...
void sample_pre(poly_matrix x, poly_matrix A_m, poly_matrix T, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, poly h_inv)
	...
	// Sample a perturbation p in R^(m+1)
	signed_scalar p_coeffs[PARAM_N * (PARAM_M+1)];
...
// approx_sample_pre with a target u
void approx_sample_pre_target(poly_matrix x, poly_matrix A_m, poly_matrix T, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, poly h_inv, poly u)
	{
	// Sample a perturbation p in R^m
	signed_scalar p_coeffs[PARAM_N * PARAM_M];
	signed_poly_matrix p = p_coeffs;
	
	approx_sample_perturb(p, cplx_T, sch_comp);
	...
	matrix_crt_representation((poly_matrix) p, PARAM_Mprime, 1, LOG_R);
	...
	multiply_by_A_approx(v, A_m, (poly_matrix) p);
	...
	matrix_crt_representation((poly_matrix) z, PARAM_D * (PARAM_K - PARAM_L), 1, LOG_R);
	...
	add_poly(x, x, (poly) p, PARAM_N * PARAM_Mprime - 1);
	// Reduce x mod q one final time but keep it in the CRT domain
	freeze_poly(x, PARAM_N * PARAM_Mprime - 1);

	free(zero);
	}


void approx_sample_pre_2(poly_matrix x, poly_matrix A, poly_matrix R, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, poly_matrix u)
{
	// Sample a perturbation p in R^m
	signed_scalar p_coeffs[PARAM_N * (PARAM_M+1)];
	signed_poly_matrix p = p_coeffs;
	...
	matrix_crt_representation((poly_matrix) p, (PARAM_M+1), 1, LOG_R);
	
	// v <- u - a^T * p (in the CRT domain)

	scalar* v_coeffs = malloc(PARAM_N * sizeof(scalar));
	poly v = v_coeffs;
	memset(v, 0, PARAM_N * sizeof(scalar));

	scalar *zero_coeffs = malloc(PARAM_N * sizeof(scalar));
	poly_matrix zero = zero_coeffs;
	zero_poly(zero, PARAM_N - 1);

	mul_crt_poly_matrix(v, A, (poly_matrix) p, 1, PARAM_M+1, 1, LOG_R);

	sub_poly(v, zero, v, PARAM_N - 1);
	add_poly(v, v, u, PARAM_N - 1);	

	
	// Put v back into the normal domain
	coeffs_representation(v, LOG_R);	
	
	// Sample z_2 from the G-lattice with target v
	signed_scalar *z_2_coeffs = malloc(PARAM_N * PARAM_M * sizeof(signed_scalar));
	signed_poly_matrix z_2 = z_2_coeffs;	
	
	ring_sample_G_cut(z_2, v);

	// Make sure z_2 has positive coefficients and put it in the CRT domain
	for(int i = 0 ; i < PARAM_N * PARAM_M ; ++i)
	{
		z_2[i] += PARAM_Q;
	}
	
	matrix_crt_representation((poly_matrix) z_2, PARAM_M, 1, LOG_R);

	// // x <- p + R * z_2 (in the CRT domain)
	mul_crt_poly_matrix(x, R, (poly_matrix) z_2, PARAM_M+1, PARAM_M, 1, LOG_R);	
	
	add_poly(x, x, (poly) p, PARAM_N * PARAM_M);

	// Reduce x mod q one final time but keep it in the CRT domain
	freeze_poly(x, PARAM_N * PARAM_M);
...
*/
void Extract2(poly_matrix x, poly_matrix A, poly_matrix u, poly_matrix T, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, scalar *id)
	{
...
void Setup2(poly_matrix A, poly_matrix T, cplx_poly_matrix cplx_T, cplx_poly_matrix sch_comp, poly_matrix u)
	{
