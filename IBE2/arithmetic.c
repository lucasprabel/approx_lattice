...
void multiply_by_A_approx(poly_matrix y, poly_matrix A, poly_matrix x)
	...
	mul_crt_poly_matrix(y, A, x_d, PARAM_D, PARAM_Mprime - PARAM_D, 1, LOG_R);
...
*/
void construct_A_m(poly_matrix A, scalar *h)
	{
	crt_representation(h, LOG_R);
	for(int i = 0 ; i < PARAM_M + 1 ; i++){
		scalar *a_h_i_coeffs = malloc(PARAM_N * sizeof(scalar));
		poly a_h_i = a_h_i_coeffs;
		mul_crt_poly_matrix(a_h_i, &A[i*PARAM_N], h, 1, 1, 1, LOG_R);
		memcpy(&A[i*PARAM_N], a_h_i, PARAM_N * sizeof(scalar));
	}
	coeffs_representation(h, LOG_R);
	}
